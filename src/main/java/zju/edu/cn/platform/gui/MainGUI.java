...
public class MainGUI {
    ...
    private zju.edu.cn.platform.redundancy.gui.EdgeGUI reEdgeGUI;
    ...
    private void addProblem() {
        ...
        JLabel title = new JLabel("<html><body><div><span style='color:#ffffff;font-size:18px;font-family:黑体;'>&nbsp;&nbsp;&nbsp;&nbsp;ZJUEdgeSim</span>" +
                "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='color:#ffffff;font-size:14px;font-family:黑体;'>A Simulator for Edge Computing</span></div></body></html>");
    ...
    private ImageIcon getImageIcon(String path, int width, int height) {
        Image scaledImage = Toolkit.getDefaultToolkit().getImage(PlatformUtils.getPathInResources(path))
                    ...
                    public void run() {
                        try {
                            if (problemType == PlatformUtils.SERVICE_REDUNDANCY) {
                                reEdgeGUI = new zju.edu.cn.platform.redundancy.gui.EdgeGUI(frame.getContentPane().getWidth(), frame.getContentPane().getHeight());
                                frame.getContentPane().removeAll();
                                frame.add(reEdgeGUI.getFrame());
                                frame.revalidate();
                                frame.repaint();
                                reEdgeGUI.generateConfig();
                                reEdgeGUI.convertJsonConfig();
//                                String configContent = PlatformUtils.readFile(
//                                        PlatformUtils.getPathInResources(
//                                                "default_config/redundancy.json"));
//                                if (configContent.length() != 0) {
//                                    reEdgeGUI.getJsonConfig();
//                                    reEdgeGUI.convertJsonConfig();
//                                }
                                ...
                                if (PlatformUtils.PROBLEM_TYPE == PlatformUtils.RESOURCE_ALLOC_AND_SCHEDULING_PROBLEM) {
                                    String configContent = PlatformUtils.readFile(
                                            PlatformUtils.getPathInResources(
                                                    "default_config/res_alloc_default_sim_config.json"));
                                    if (configContent.length() != 0) {
                                        edgeGUI.setJsonConfig(JSONObject.parseObject(configContent, JsonConfig.class));
                                        edgeGUI.convertJsonConfig();
                                        edgeGUI.addLegends();
                                    }
                                } else if (PlatformUtils.PROBLEM_TYPE == PlatformUtils.BURST_LOAD_EVACUATION) {
                                    String stringConfigContent = PlatformUtils.readFile(
                                            PlatformUtils.getPathInResources("default_config/burst_load_config.json"));
                                    if (stringConfigContent.length() != 0) {
                                        edgeGUI.setJsonConfig(JSONObject.parseObject(stringConfigContent, JsonConfig.class));
                                        edgeGUI.convertJsonConfig();
                                        edgeGUI.addLegends();
                                    }
                                }
